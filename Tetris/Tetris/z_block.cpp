...
Z_Block::Z_Block(short colour) : Tetromino(colour) {
	type = BlockName::Z_Block;
...
void Z_Block::checkRotate(bool clockwise) {
...
void Z_Block::createBlock(std::vector<Unit>& units, COORD sprite[]) {
	Unit new_unit;
	for (char i = 0; i < 4; i++) {
...
bool Z_Block::overlappingBlock(std::vector<Unit>& virtual_units) {
...
void Z_Block::rotateClockwise(std::vector<Unit>& virtual_units, COORD new_sprite[], COORD shift) {
	removeGhostBlock();
	removeBlock();
	subtractSquareFrame();
	for (int i = 0; i < 4; i++) {
