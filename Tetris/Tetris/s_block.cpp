...
S_Block::S_Block(short colour) : Tetromino(colour) {
	type = BlockName::S_Block;
...
void S_Block::checkRotate(bool clockwise) {
...
void S_Block::createBlock(std::vector<Unit>& units, COORD sprite[]) {
	Unit new_unit;
	for (char i = 0; i < 4; i++) {
...
bool S_Block::overlappingBlock(std::vector<Unit>& virtual_units) {
...
void S_Block::rotateClockwise(std::vector<Unit>& virtual_units, COORD new_sprite[], COORD shift) {
	removeGhostBlock();
	removeBlock();
	subtractSquareFrame();
	for (int i = 0; i < 4; i++) {
