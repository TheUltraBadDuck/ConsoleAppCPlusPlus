...
J_Block::J_Block(short colour) : Tetromino(colour) {
	type = BlockName::J_Block;
...
void J_Block::checkRotate(bool clockwise) {
...
void J_Block::createBlock(std::vector<Unit>& units, COORD sprite[]) {
	Unit new_unit;
	for (char i = 0; i < 4; i++) {
...
bool J_Block::overlappingBlock(std::vector<Unit>& virtual_units) {
...
void J_Block::rotateClockwise(std::vector<Unit>& virtual_units, COORD new_sprite[], COORD shift) {
	removeGhostBlock();
	removeBlock();
	subtractSquareFrame();
	for (int i = 0; i < 4; i++) {
