...
I_Block::I_Block(short colour) : Tetromino(colour) {
	type = BlockName::I_Block;
...
void I_Block::checkRotate(bool clockwise) {
...
void I_Block::createBlock(std::vector<Unit>& units, COORD sprite[]) {
	Unit new_unit;
	for (char i = 0; i < 4; i++) {
...
bool I_Block::overlappingBlock(std::vector<Unit>& virtual_units) {
...
void I_Block::rotateClockwise(std::vector<Unit>& virtual_units, COORD new_sprite[], COORD shift) {
	removeGhostBlock();
	removeBlock();
	subtractSquareFrame();
	for (int i = 0; i < 4; i++) {
	...
	setApparentX(center_apparent.X + shift.X);
	setApparentY(center_apparent.Y + shift.Y);
...
//for (int i = 0; i < 4; i++) {
//	gotoXY(0, i);
//	std::cout << virtual_units[i].apparent.X << "  " << virtual_units[i].apparent.Y << "  ";
//	gotoXY(10, i);
//	std::cout << units[i].apparent.X << "  " << units[i].apparent.Y << "  ";
//}
