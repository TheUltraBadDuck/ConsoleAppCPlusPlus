...
L_Block::L_Block(short colour) : Tetromino(colour) {
	type = BlockName::L_Block;
...
void L_Block::checkRotate(bool clockwise) {
...
void L_Block::createBlock(std::vector<Unit>& units, COORD sprite[]) {
	Unit new_unit;
	for (char i = 0; i < 4; i++) {
...
bool L_Block::overlappingBlock(std::vector<Unit>& virtual_units) {
...
void L_Block::rotateClockwise(std::vector<Unit>& virtual_units, COORD new_sprite[], COORD shift) {
	removeGhostBlock();
	removeBlock();
	subtractSquareFrame();
	for (int i = 0; i < 4; i++) {
